...
export class AcornClient {
  ...
  private async retryOperation<T>(
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (isServerError) {
          logger.warn(
            `Attempt ${i + 1} failed, retrying in ${this.RETRY_DELAY / 1000}s...`
          );
    ...
    try {
      ...
      for (const meeting of meetings) {
        ...
        const isTargetLecture =
          teachMethod === "LEC" &&
          (!config.lectureSections?.length ||
            config.lectureSections.includes(sectionNo));

        const isTargetTutorial =
          teachMethod === "TUT" &&
          (!config.tutorialSections?.length ||
            config.tutorialSections.includes(sectionNo));
        ...
        if (isTargetLecture || isTargetTutorial) {
          if (enrollmentSpaceAvailable > 0) {
            ...
            logger.debug(
              `${config.courseCode} has no space left for ${displayName}`
            );
          }
        }
      }
  ...
  private async enrollCourse(
    ...
    const enrollUrl =
      "https://acorn.utoronto.ca/sws/rest/enrolment/course/modify";
    ...
    const body = {
      activeCourse: {
        ...
        lecture: teachMethod === "LEC" ? { sectionNo: `LEC,${sectionNo}` } : {},
        tutorial:
          teachMethod === "TUT" ? { sectionNo: `TUT,${sectionNo}` } : {},
        practical: {},
      },
      eligRegParams: {
        postCode: "ASCRSHBSC",
    ...
    try {
      const response = await this.retryOperation(async () => {
        const headers = await this.getHeaders();
        return this.page!.request.post(enrollUrl, {
          headers,
          data: body,
        });
      });
      ...
      if (result.messages?.errors?.length > 0) {
        throw new Error(
          `Enrollment error: ${result.messages.errors.join(", ")}`
        );
      }

      logger.info(
        `Successfully enrolled in ${config.courseCode} section ${sectionNo}`
      );
    } catch (error) {
